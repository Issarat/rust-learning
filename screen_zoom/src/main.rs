...
    let Some(screen) = app.last_screen.as_ref() else { return; };
...
                        Key::Character(ref c) if c == "+" || c == "=" => app.zoom += 0.5,
                        Key::Character(ref c) if c == "-" => app.zoom -= 0.5,
                        _ => {}
                    }
            ...
            Event::AboutToWait => {
                if app.active {
                    ...
                    if app.last_screen.is_none() || dist > 5.0 {
                        ...
                        window.set_outer_position(LogicalPosition::new(app.window_pos.0, app.window_pos.1));
                        force_update(&window);
                    }
